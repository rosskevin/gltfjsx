/**
 * Q: Why copy this at all if we can just import it?  I guess after we refactor and figure it out, we can always just remove it!
 * 
 * @source from https://github.com/mrdoob/three.js/blob/dev/examples/jsm/loaders/GLTFLoader.js
 */
...
class BaseExtension {
  parser: GLTFParser
  name: string
  constructor(parser: GLTFParser, name: string) {
    this.parser = parser
    this.name = name
  }
}
...
interface MaterialExtension {
  ...
  extendMaterialParams(materialIndex: number, materialParams: MaterialParams): Promise<[]> | Promise<void>
}

interface TextureExtension {
...
class GLTFLoader extends Loader {
  ...
  ktx2Loader?: Loader
  meshoptDecoder: any
  pluginCallbacks: Array<(parser: any) => any>
  ...
  register(callback: (parser: GLTFParser) => BaseExtension) {
...
class GLTFRegistry<R extends object> {
  objects: { [key: string]: R }
  constructor(){
    this.objects = {}
  }
 ...
 */
class GLTFLightsExtension extends BaseExtension {
  cache: { refs: {}; uses: {} }
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_LIGHTS_PUNCTUAL)
 ...
 */
class GLTFMaterialsEmissiveStrengthExtension extends BaseExtension /* not quite MaterialExtension */{
  constructor(parser: GLTFParser) {
    super(parser,EXTENSIONS.KHR_MATERIALS_EMISSIVE_STRENGTH)
  }
 ...
 */
class GLTFMaterialsClearcoatExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_MATERIALS_CLEARCOAT)
  }
 ...
 */
class GLTFMaterialsDispersionExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_MATERIALS_DISPERSION)
  }
 ...
 */
class GLTFMaterialsIridescenceExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super( parser,EXTENSIONS.KHR_MATERIALS_IRIDESCENCE)
  }
 ...
 */
class GLTFMaterialsSheenExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_MATERIALS_SHEEN)
  }
 ...
 */
class GLTFMaterialsTransmissionExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_MATERIALS_TRANSMISSION)
  }
 ...
 */
class GLTFMaterialsVolumeExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_MATERIALS_VOLUME)
  }
 ...
 */
class GLTFMaterialsIorExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_MATERIALS_IOR)
  }
 ...
 */
class GLTFMaterialsSpecularExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_MATERIALS_SPECULAR)
  }
 ...
 */
class GLTFMaterialsBumpExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser,  EXTENSIONS.EXT_MATERIALS_BUMP)
  }
 ...
 */
class GLTFMaterialsAnisotropyExtension extends BaseExtension implements MaterialExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_MATERIALS_ANISOTROPY)
  }
 ...
 */
class GLTFTextureBasisUExtension extends BaseExtension implements TextureExtension {
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.KHR_TEXTURE_BASISU)
  }
 ...
 */
class GLTFTextureWebPExtension extends BaseExtension implements TextureExtension{
  isSupported: Promise<boolean> | null
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.EXT_TEXTURE_WEBP)
 ...
 */
class GLTFTextureAVIFExtension extends BaseExtension implements TextureExtension {
  isSupported: Promise<boolean> | null
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.EXT_TEXTURE_AVIF)
    this.isSupported = null
  }
 ...
 */
class GLTFMeshoptCompression extends BaseExtension{
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.EXT_MESHOPT_COMPRESSION)
  }
 ...
 *
 */
class GLTFMeshGpuInstancing extends BaseExtension{
  constructor(parser: GLTFParser) {
    super(parser, EXTENSIONS.EXT_MESH_GPU_INSTANCING)
  }
...
class GLTFParser {
   ...
   */
  loadTexture(textureIndex: number): Texture {
    const json = this.json
    const options = this.options
    const textureDef = json.textures[textureIndex]
  ...
  loadTextureImage(textureIndex, sourceIndex, loader): Promise<Texture> {
    const parser = this
    const json = this.json
  ...
  loadImageSource(sourceIndex, loader): Promise<Texture> {
    const parser = this
    const json = this.json
    const options = this.options
  ...
  loadMaterial(materialIndex) {
    ...
    if (materialExtensions[EXTENSIONS.KHR_MATERIALS_UNLIT]) {
      ...
      materialType = this._invokeOne(function (ext) {
        return ext.getMaterialType && ext.getMaterialType(materialIndex: number)
      })
      ...
      pending.push(
        Promise.all(
          this._invokeAll(function (ext) {
            return (
              ext.extendMaterialParams && ext.extendMaterialParams(materialIndex: number, materialParams: MaterialParams)
            )
          }),
        ),
      )
